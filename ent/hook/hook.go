...
// The StorefrontFunc type is an adapter to allow the use of ordinary
// function as Storefront mutator.
type StorefrontFunc func(context.Context, *ent.StorefrontMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f StorefrontFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.StorefrontMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.StorefrontMutation", m)
}
